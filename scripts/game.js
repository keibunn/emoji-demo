...
b=0;10>b;b++)for(var d=0;8>d;d++)if(l[b][d].filled){var c=0;a:for(;c<a;c++){var e=f[c];if(e.pos.x===d&&e.pos.y===b){e.depth=0;break a}}}y="gameover";p.add.rectangle(0,0,config.width,config.height,0).setOrigin(0).alpha=.8;p.add.text(187.5,300,"GAMEOVER",{fontFamily:"PoetsenOne",fontSize:45,align:"center",color:"#FFFFFF"}).setOrigin(.5);p.add.text(187.5,360,"SCORE: "+player_data.score,{fontFamily:"PoetsenOne",fontSize:32,align:"center",color:"#FFFFFF"}).setOrigin(.5);draw_button(187.5,450,"restart",p);draw_button(382,
520,"menu",p);localStorage.removeItem("redfoc_onet_array");player_data.drop_mode=0;player_data.score=0;last_array=null;localStorage.setItem("redfoc_onet_data",JSON.stringify(player_data))}function W(){var a={arr:l,data:player_data};last_array=JSON.parse(JSON.stringify(l));localStorage.setItem("redfoc_onet_array",JSON.stringify(a))}
...
function timeUpGameOver(){
    ...
    // 显示时间到结算信息，调整位置避免重叠
    var timeUpText = p.add.text(187.5,200,"TIME'S UP!",{fontFamily:"PoetsenOne",fontSize:36,align:"center",color:"#FF6B6B"}).setOrigin(.5);
    timeUpText.depth = 1001;
    
    var finalScoreLabel = p.add.text(187.5,280,"FINAL SCORE",{fontFamily:"PoetsenOne",fontSize:28,align:"center",color:"#FFFFFF"}).setOrigin(.5);
    finalScoreLabel.depth = 1001;
    
    var scoreText = p.add.text(187.5,340,String(finalScore),{fontFamily:"PoetsenOne",fontSize:56,align:"center",color:"#FFD93D"}).setOrigin(.5);
    scoreText.depth = 1001;
    
    var restartBtn = draw_button(187.5,420,"restart",p);
    restartBtn.depth = 1001;
    
    var menuBtn = draw_button(187.5,490,"menu",p);
    menuBtn.depth = 1001;
