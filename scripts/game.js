...
function timeUpGameOver(){
    ...
    // 添加半透明遮罩，深度设置为1000确保在最上层
    var overlay = p.add.rectangle(0,0,config.width,config.height,0).setOrigin(0);
    overlay.alpha=.8;
    overlay.setDepth(1000);
    
    // 添加文本，深度设置为1001确保在遮罩之上
    var timeUpText = p.add.text(360,350,"TIME'S UP!",{fontFamily:"PoetsenOne",fontSize:55,align:"center",color:"#FF6B6B"}).setOrigin(.5);
    timeUpText.setDepth(1001);
    
    var finalScoreLabel = p.add.text(360,420,"FINAL SCORE",{fontFamily:"PoetsenOne",fontSize:45,align:"center",color:"#FFFFFF"}).setOrigin(.5);
    finalScoreLabel.setDepth(1001);
    
    var scoreText = p.add.text(360,480,String(finalScore),{fontFamily:"PoetsenOne",fontSize:65,align:"center",color:"#FFD93D"}).setOrigin(.5);
    scoreText.setDepth(1001);
    
    // 添加按钮，深度设置为1001
    var restartBtn = draw_button(360,610,"restart",p);
    restartBtn.setDepth(1001);
    var menuBtn = draw_button(360,700,"menu",p);
...
function updateTimeDisplay(){
    if(gameEnded) return; // 游戏结束后停止更新时间
