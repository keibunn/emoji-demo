...
function timeUpGameOver(){
    ...
    // 显示时间到结算信息，调整位置避免重叠
    var timeUpText = p.add.text(360,250,"TIME'S UP!",{fontFamily:"PoetsenOne",fontSize:50,align:"center",color:"#FF6B6B"}).setOrigin(.5);
    timeUpText.depth = 1001;
    
    var finalScoreLabel = p.add.text(360,350,"FINAL SCORE",{fontFamily:"PoetsenOne",fontSize:40,align:"center",color:"#FFFFFF"}).setOrigin(.5);
    finalScoreLabel.depth = 1001;
    
    var scoreText = p.add.text(360,450,String(finalScore),{fontFamily:"PoetsenOne",fontSize:80,align:"center",color:"#FFD93D"}).setOrigin(.5);
    scoreText.depth = 1001;
    
    var restartBtn = draw_button(360,580,"restart",p);
    restartBtn.depth = 1001;
    
    var menuBtn = draw_button(360,680,"menu",p);
    menuBtn.depth = 1001;
...
var N=this;PokiSDK.gameplayStart();this.add.sprite(0,0,"background2").setOrigin(0);var y="play",n=0,p=this,E,O=!1,u=this.add.group();this.add.group();var J=this.add.group(),
